...
async function testEmailChangeProtection() {
  ...
  try {
    ...
    // Wait for either profile form or potential redirect/loading
    try {
      await page.waitForSelector('input[name="email"]', { timeout: 15000 })
      await page.waitForSelector('input[name="first_name"]', { timeout: 5000 })
      await page.waitForSelector('input[name="last_name"]', { timeout: 5000 })
    } catch (error) {
      console.log('   Profile form not found immediately, taking debug screenshot...')
      await page.screenshot({ path: 'test-results/email-protection-02-debug.png' })
      throw error
    }
...
async function testAccessibility() {
  ...
  try {
    ...
    await loginAsPlayer(page, { takeScreenshot: false, port: 3005 })
    await page.goto('http://localhost:3005/profile')
